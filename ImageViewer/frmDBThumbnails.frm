VERSION 5.00
Object = "{831FDD16-0C5C-11D2-A9FC-0000F8754DA1}#2.0#0"; "MSCOMCTL.OCX"
Begin VB.Form frmDBThumbnails 
   BackColor       =   &H00000000&
   BorderStyle     =   3  'Fixed Dialog
   ClientHeight    =   6960
   ClientLeft      =   45
   ClientTop       =   45
   ClientWidth     =   9510
   ControlBox      =   0   'False
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ScaleHeight     =   6960
   ScaleWidth      =   9510
   ShowInTaskbar   =   0   'False
   Begin VB.Frame frmImage 
      BackColor       =   &H00000000&
      BorderStyle     =   0  'None
      Height          =   1515
      Index           =   23
      Left            =   7920
      TabIndex        =   24
      Top             =   5040
      Width           =   1515
      Begin VB.Image imgThumbs 
         Height          =   1500
         Index           =   23
         Left            =   0
         Stretch         =   -1  'True
         Top             =   0
         Width           =   1500
      End
   End
   Begin VB.Frame frmImage 
      BackColor       =   &H00000000&
      BorderStyle     =   0  'None
      Height          =   1515
      Index           =   22
      Left            =   6360
      TabIndex        =   23
      Top             =   5040
      Width           =   1515
      Begin VB.Image imgThumbs 
         Height          =   1500
         Index           =   22
         Left            =   0
         Stretch         =   -1  'True
         Top             =   0
         Width           =   1500
      End
   End
   Begin VB.Frame frmImage 
      BackColor       =   &H00000000&
      BorderStyle     =   0  'None
      Height          =   1515
      Index           =   21
      Left            =   4800
      TabIndex        =   22
      Top             =   5040
      Width           =   1515
      Begin VB.Image imgThumbs 
         Height          =   1500
         Index           =   21
         Left            =   0
         Stretch         =   -1  'True
         Top             =   0
         Width           =   1500
      End
   End
   Begin VB.Frame frmImage 
      BackColor       =   &H00000000&
      BorderStyle     =   0  'None
      Height          =   1515
      Index           =   20
      Left            =   3240
      TabIndex        =   21
      Top             =   5040
      Width           =   1515
      Begin VB.Image imgThumbs 
         Height          =   1500
         Index           =   20
         Left            =   0
         Stretch         =   -1  'True
         Top             =   0
         Width           =   1500
      End
   End
   Begin VB.Frame frmImage 
      BackColor       =   &H00000000&
      BorderStyle     =   0  'None
      Height          =   1515
      Index           =   19
      Left            =   1680
      TabIndex        =   20
      Top             =   5040
      Width           =   1515
      Begin VB.Image imgThumbs 
         Height          =   1500
         Index           =   19
         Left            =   0
         Stretch         =   -1  'True
         Top             =   0
         Width           =   1500
      End
   End
   Begin VB.Frame frmImage 
      BackColor       =   &H00000000&
      BorderStyle     =   0  'None
      Height          =   1515
      Index           =   18
      Left            =   120
      TabIndex        =   19
      Top             =   5040
      Width           =   1515
      Begin VB.Image imgThumbs 
         Height          =   1500
         Index           =   18
         Left            =   0
         Stretch         =   -1  'True
         Top             =   0
         Width           =   1500
      End
   End
   Begin VB.Frame frmImage 
      BackColor       =   &H00000000&
      BorderStyle     =   0  'None
      Height          =   1515
      Index           =   17
      Left            =   7920
      TabIndex        =   18
      Top             =   3480
      Width           =   1515
      Begin VB.Image imgThumbs 
         Height          =   1500
         Index           =   17
         Left            =   0
         Stretch         =   -1  'True
         Top             =   0
         Width           =   1500
      End
   End
   Begin VB.Frame frmImage 
      BackColor       =   &H00000000&
      BorderStyle     =   0  'None
      Height          =   1515
      Index           =   16
      Left            =   6360
      TabIndex        =   17
      Top             =   3480
      Width           =   1515
      Begin VB.Image imgThumbs 
         Height          =   1500
         Index           =   16
         Left            =   0
         Stretch         =   -1  'True
         Top             =   0
         Width           =   1500
      End
   End
   Begin VB.Frame frmImage 
      BackColor       =   &H00000000&
      BorderStyle     =   0  'None
      Height          =   1515
      Index           =   15
      Left            =   4800
      TabIndex        =   16
      Top             =   3480
      Width           =   1515
      Begin VB.Image imgThumbs 
         Height          =   1500
         Index           =   15
         Left            =   0
         Stretch         =   -1  'True
         Top             =   0
         Width           =   1500
      End
   End
   Begin VB.Frame frmImage 
      BackColor       =   &H00000000&
      BorderStyle     =   0  'None
      Height          =   1515
      Index           =   14
      Left            =   3240
      TabIndex        =   15
      Top             =   3480
      Width           =   1515
      Begin VB.Image imgThumbs 
         Height          =   1500
         Index           =   14
         Left            =   0
         Stretch         =   -1  'True
         Top             =   0
         Width           =   1500
      End
   End
   Begin VB.Frame frmImage 
      BackColor       =   &H00000000&
      BorderStyle     =   0  'None
      Height          =   1515
      Index           =   13
      Left            =   1680
      TabIndex        =   14
      Top             =   3480
      Width           =   1515
      Begin VB.Image imgThumbs 
         Height          =   1500
         Index           =   13
         Left            =   0
         Stretch         =   -1  'True
         Top             =   0
         Width           =   1500
      End
   End
   Begin VB.Frame frmImage 
      BackColor       =   &H00000000&
      BorderStyle     =   0  'None
      Height          =   1515
      Index           =   12
      Left            =   120
      TabIndex        =   13
      Top             =   3480
      Width           =   1515
      Begin VB.Image imgThumbs 
         Height          =   1500
         Index           =   12
         Left            =   0
         Stretch         =   -1  'True
         Top             =   0
         Width           =   1500
      End
   End
   Begin VB.Frame frmImage 
      BackColor       =   &H00000000&
      BorderStyle     =   0  'None
      Height          =   1515
      Index           =   11
      Left            =   7920
      TabIndex        =   12
      Top             =   1920
      Width           =   1515
      Begin VB.Image imgThumbs 
         Height          =   1500
         Index           =   11
         Left            =   0
         Stretch         =   -1  'True
         Top             =   0
         Width           =   1500
      End
   End
   Begin VB.Frame frmImage 
      BackColor       =   &H00000000&
      BorderStyle     =   0  'None
      Height          =   1515
      Index           =   10
      Left            =   6360
      TabIndex        =   11
      Top             =   1920
      Width           =   1515
      Begin VB.Image imgThumbs 
         Height          =   1500
         Index           =   10
         Left            =   0
         Stretch         =   -1  'True
         Top             =   0
         Width           =   1500
      End
   End
   Begin VB.Frame frmImage 
      BackColor       =   &H00000000&
      BorderStyle     =   0  'None
      Height          =   1515
      Index           =   9
      Left            =   4800
      TabIndex        =   10
      Top             =   1920
      Width           =   1515
      Begin VB.Image imgThumbs 
         Height          =   1500
         Index           =   9
         Left            =   0
         Stretch         =   -1  'True
         Top             =   0
         Width           =   1500
      End
   End
   Begin VB.Frame frmImage 
      BackColor       =   &H00000000&
      BorderStyle     =   0  'None
      Height          =   1515
      Index           =   8
      Left            =   3240
      TabIndex        =   9
      Top             =   1920
      Width           =   1515
      Begin VB.Image imgThumbs 
         Height          =   1500
         Index           =   8
         Left            =   0
         Stretch         =   -1  'True
         Top             =   0
         Width           =   1500
      End
   End
   Begin VB.Frame frmImage 
      BackColor       =   &H00000000&
      BorderStyle     =   0  'None
      Height          =   1515
      Index           =   7
      Left            =   1680
      TabIndex        =   8
      Top             =   1920
      Width           =   1515
      Begin VB.Image imgThumbs 
         Height          =   1500
         Index           =   7
         Left            =   0
         Stretch         =   -1  'True
         Top             =   0
         Width           =   1500
      End
   End
   Begin VB.Frame frmImage 
      BackColor       =   &H00000000&
      BorderStyle     =   0  'None
      Height          =   1515
      Index           =   6
      Left            =   120
      TabIndex        =   7
      Top             =   1920
      Width           =   1515
      Begin VB.Image imgThumbs 
         Height          =   1500
         Index           =   6
         Left            =   0
         Stretch         =   -1  'True
         Top             =   0
         Width           =   1500
      End
   End
   Begin VB.Frame frmImage 
      BackColor       =   &H00000000&
      BorderStyle     =   0  'None
      Height          =   1515
      Index           =   5
      Left            =   7920
      TabIndex        =   6
      Top             =   360
      Width           =   1515
      Begin VB.Image imgThumbs 
         Height          =   1500
         Index           =   5
         Left            =   0
         Stretch         =   -1  'True
         Top             =   0
         Width           =   1500
      End
   End
   Begin VB.Frame frmImage 
      BackColor       =   &H00000000&
      BorderStyle     =   0  'None
      Height          =   1515
      Index           =   4
      Left            =   6360
      TabIndex        =   5
      Top             =   360
      Width           =   1515
      Begin VB.Image imgThumbs 
         Height          =   1500
         Index           =   4
         Left            =   0
         Stretch         =   -1  'True
         Top             =   0
         Width           =   1500
      End
   End
   Begin VB.Frame frmImage 
      BackColor       =   &H00000000&
      BorderStyle     =   0  'None
      Height          =   1515
      Index           =   3
      Left            =   4800
      TabIndex        =   4
      Top             =   360
      Width           =   1515
      Begin VB.Image imgThumbs 
         Height          =   1500
         Index           =   3
         Left            =   0
         Stretch         =   -1  'True
         Top             =   0
         Width           =   1500
      End
   End
   Begin VB.Frame frmImage 
      BackColor       =   &H00000000&
      BorderStyle     =   0  'None
      Height          =   1515
      Index           =   2
      Left            =   3240
      TabIndex        =   3
      Top             =   360
      Width           =   1515
      Begin VB.Image imgThumbs 
         Height          =   1500
         Index           =   2
         Left            =   0
         Stretch         =   -1  'True
         Top             =   0
         Width           =   1500
      End
   End
   Begin VB.Frame frmImage 
      BackColor       =   &H00000000&
      BorderStyle     =   0  'None
      Height          =   1515
      Index           =   1
      Left            =   1680
      TabIndex        =   2
      Top             =   360
      Width           =   1515
      Begin VB.Image imgThumbs 
         Height          =   1500
         Index           =   1
         Left            =   0
         Stretch         =   -1  'True
         Top             =   0
         Width           =   1500
      End
   End
   Begin VB.Frame frmImage 
      BackColor       =   &H00000000&
      BorderStyle     =   0  'None
      Height          =   1515
      Index           =   0
      Left            =   120
      TabIndex        =   1
      Top             =   360
      Width           =   1515
      Begin VB.Image imgThumbs 
         Height          =   1500
         Index           =   0
         Left            =   0
         Stretch         =   -1  'True
         Top             =   0
         Width           =   1500
      End
   End
   Begin MSComctlLib.Toolbar tlbThumbs 
      Align           =   1  'Align Top
      Height          =   330
      Left            =   0
      TabIndex        =   0
      Top             =   0
      Width           =   9510
      _ExtentX        =   16775
      _ExtentY        =   582
      ButtonWidth     =   2540
      ButtonHeight    =   582
      Style           =   1
      TextAlignment   =   1
      ImageList       =   "ImageList1"
      _Version        =   393216
      BeginProperty Buttons {66833FE8-8583-11D1-B16A-00C0F0283628} 
         NumButtons      =   6
         BeginProperty Button1 {66833FEA-8583-11D1-B16A-00C0F0283628} 
            Enabled         =   0   'False
            Caption         =   "Previous Page"
            Key             =   "back"
            ImageIndex      =   1
         EndProperty
         BeginProperty Button2 {66833FEA-8583-11D1-B16A-00C0F0283628} 
            Caption         =   "Next page"
            Key             =   "nextpage"
            ImageIndex      =   2
         EndProperty
         BeginProperty Button3 {66833FEA-8583-11D1-B16A-00C0F0283628} 
            Style           =   3
         EndProperty
         BeginProperty Button4 {66833FEA-8583-11D1-B16A-00C0F0283628} 
            Caption         =   "Slide Show"
            Key             =   "slideshow"
            ImageIndex      =   3
         EndProperty
         BeginProperty Button5 {66833FEA-8583-11D1-B16A-00C0F0283628} 
            Style           =   3
         EndProperty
         BeginProperty Button6 {66833FEA-8583-11D1-B16A-00C0F0283628} 
            Caption         =   "Close"
            Key             =   "close"
            ImageIndex      =   4
         EndProperty
      EndProperty
   End
   Begin MSComctlLib.ImageList ImageList1 
      Left            =   1440
      Top             =   6240
      _ExtentX        =   1005
      _ExtentY        =   1005
      BackColor       =   -2147483643
      ImageWidth      =   16
      ImageHeight     =   16
      MaskColor       =   12632256
      _Version        =   393216
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         NumListImages   =   4
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmDBThumbnails.frx":0000
            Key             =   ""
         EndProperty
         BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmDBThumbnails.frx":0454
            Key             =   ""
         EndProperty
         BeginProperty ListImage3 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmDBThumbnails.frx":08A8
            Key             =   ""
         EndProperty
         BeginProperty ListImage4 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmDBThumbnails.frx":1184
            Key             =   ""
         EndProperty
      EndProperty
   End
   Begin VB.Image imgTemp 
      Height          =   1575
      Left            =   5640
      Top             =   5160
      Visible         =   0   'False
      Width           =   2295
   End
   Begin VB.Label lblPageNum 
      Alignment       =   2  'Center
      BackStyle       =   0  'Transparent
      Caption         =   "Page 0 of 0"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      ForeColor       =   &H00FFFFFF&
      Height          =   255
      Left            =   195
      TabIndex        =   25
      Top             =   6600
      Width           =   9255
   End
End
Attribute VB_Name = "frmDBThumbnails"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
Option Explicit
Public DBAImgRS As ADODB.Recordset
Public ImgID As Integer
Public ImgType As String
Public ImgSubType As String
Public ImgSubMinType As String
Dim strPath As String
Dim NumOfPics As Integer
Dim Pics() As String
Dim First As Integer
Dim Page As Integer
Dim Pages As Integer


Private Sub Form_Load()
First = 0
Page = 0
Pages = 0
Me.Height = 7050
Me.Width = 9600
Me.Move (frmMain.Width / 2) - (Me.Width / 2), (frmMain.Height / 2) - ((Me.Height / 2) + 380)

End Sub

Public Sub CreateThumbs(iImgType As String, iImgSubType As String, iImgSubMinType As String)
Dim SqlStr As String
ImgType = iImgType
ImgSubType = iImgSubType
ImgSubMinType = iImgSubMinType
Set DBAImgRS = New ADODB.Recordset
SqlStr = "Select * from imagefile where imagetype = '" & ImgType & _
"' and subtype = '" & ImgSubType & "' and submintype ='" & ImgSubMinType & "'"
DBAImgRS.Open SqlStr, DB, adOpenStatic, adLockOptimistic
If DBAImgRS.RecordCount <> 0 Then
NumOfPics = DBAImgRS.RecordCount
Me.Show
DBAImgRS.MoveFirst
Unload frmSelectImageTypes
DisplayThumbs
ElseIf DBAImgRS.RecordCount = 0 Then
MsgBox "No Image for Type " & ImgType
frmMain.mnuDBThumNails.Enabled = True
End If
End Sub

Public Sub DisplayThumbs()
Dim ThumbNum As Integer
Page = 0
Pages = 0
tlbThumbs.Buttons("back").Enabled = False
tlbThumbs.Buttons("nextpage").Enabled = False
Pages = NumOfPics / 24
If NumOfPics <= 24 Then
 tlbThumbs.Buttons("nextpage").Enabled = False
ElseIf NumOfPics > 24 Then
 tlbThumbs.Buttons("nextpage").Enabled = True
End If
If Pages * 24 < NumOfPics Then
 Pages = Pages + 1
End If
Page = Page + 1
lblPageNum.Caption = "Pages : " & Page & " of " & Pages
 tlbThumbs.Buttons("close").Enabled = False
 First = 0
 ClearThumbs
  For ThumbNum = 0 To 23
    DisplayImg ThumbNum
    'Loop
  If First >= NumOfPics Then
    tlbThumbs.Buttons("close").Enabled = True
    Exit Sub
  End If
  Next ThumbNum
 tlbThumbs.Buttons("close").Enabled = True
End Sub
Private Sub ClearThumbs()
Dim j As Integer
For j = 0 To 23
 imgThumbs(j).Picture = LoadPicture()
Next
End Sub
Private Sub DisplayImg(imgIndex As Integer)
Dim imgWidth As Integer
Dim imgHeight As Integer
Dim NewH As Integer
Dim NewW As Integer

On Error GoTo openerror
      First = First + 1
      strPath = ReadFromDba(DBAImgRS!Image, "", DBAImgRS!comp)
      imgTemp.Picture = LoadPicture(strPath)
      imgWidth = imgTemp.Width 'imgThumb(imgIndex).Width
      imgHeight = imgTemp.Height 'imgThumb(imgIndex).Height
      ThumbSize imgWidth, imgHeight, NewW, NewH
      imgThumbs(imgIndex).Move (frmImage(imgIndex).Width - NewW) / 2, (frmImage(imgIndex).Height - NewH) / 2, NewW, NewH
      imgThumbs(imgIndex).Picture = LoadPicture(strPath)
      imgThumbs(imgIndex).ToolTipText = "Click to Open Image"
      imgThumbs(imgIndex).Tag = DBAImgRS!imageid
      DoEvents
      Kill strPath
      DBAImgRS.MoveNext
      Exit Sub
openerror:
 imgThumbs(imgIndex).Picture = LoadPicture()
 imgThumbs(imgIndex).Tag = ""
 First = First - 1

End Sub

Private Sub imgThumbs_MouseDown(Index As Integer, Button As Integer, Shift As Integer, X As Single, Y As Single)

Dim SingleImgRs As ADODB.Recordset
Dim TempFile As String
On Error Resume Next

If Button = 1 Then
 If imgThumbs(Index).Tag <> "" Then
  ImgID = imgThumbs(Index).Tag
  Set SingleImgRs = New ADODB.Recordset
  SingleImgRs.Open "Select * from imagefile where imageid = " & ImgID, DB, adOpenStatic, adLockOptimistic
  TempFile = ReadFromDba(SingleImgRs!Image, "", SingleImgRs!comp)
    frmMain.OpenImage TempFile
    Kill TempFile
 End If
 
ElseIf Button = 2 Then
 If imgThumbs(Index).Tag <> "" Then
  ImgID = imgThumbs(Index).Tag
  PopupMenu frmMain.mnuDbaThumb
 End If 'ImgThumb
End If 'Button
End Sub

Private Sub tlbThumbs_ButtonClick(ByVal Button As MSComctlLib.Button)
Select Case Button.Key
    Case "back"
         BackAPage
    Case "nextpage"
         NextPage
    Case "slideshow"
        With frmDBSlideShow
         .CreateSlideShow ImgType, ImgSubType, ImgSubMinType
        End With
    Case "close"
     Unload Me
     frmMain.mnuDBThumNails.Enabled = True
End Select
End Sub
Private Sub ThumbSize(iwidth As Integer, iheight As Integer, iNewW As Integer, iNewH As Integer)
Dim T As Integer
iNewW = iwidth: iNewH = iheight
T = 1
Do While iNewW > 1500 Or iNewH > 1500
 iNewW = Int(iwidth / T)
 iNewH = Int(iheight / T)
 T = T + 1
Loop
End Sub

Private Sub BackAPage()
Dim j As Integer
Dim ImgNum As Integer
Dim NewImgNum As Integer
For j = 1 To First + 24
  DBAImgRS.MovePrevious
Next
First = 0
 tlbThumbs.Buttons("close").Enabled = False

For j = 0 To 23
   DisplayImg j
Next j
 tlbThumbs.Buttons("close").Enabled = True
First = 0
tlbThumbs.Buttons("nextpage").Enabled = True
Page = Page - 1
If Page = 1 Then
 tlbThumbs.Buttons("back").Enabled = False
End If
lblPageNum.Caption = "Pages : " & Page & " of " & Pages
End Sub

Private Sub NextPage()
Dim ImgNum As Integer
Dim j As Integer
First = 0
 tlbThumbs.Buttons("close").Enabled = False
For j = 0 To 23
   DisplayImg j
Next j
 tlbThumbs.Buttons("close").Enabled = True
tlbThumbs.Buttons("back").Enabled = True
Page = Page + 1
If Page = Pages Then
 tlbThumbs.Buttons("nextpage").Enabled = False
End If
lblPageNum.Caption = "Pages : " & Page & " of " & Pages
End Sub

Public Sub DelImgFromDBA()
Dim DelImgRs As ADODB.Recordset
Set DelImgRs = New ADODB.Recordset
DelImgRs.Open "Select image from imagefile where imageid = " & ImgID, DB, adOpenStatic, adLockOptimistic
If DelImgRs.RecordCount <> 0 Then
 DelImgRs.Delete
 CreateThumbs ImgType, ImgSubType, ImgSubMinType
End If
End Sub

Public Sub ExportImage()
Dim ExportImgRs As ADODB.Recordset
Dim TempFile As String
Dim FileStr As String
Dim Pos As Long
Set ExportImgRs = New ADODB.Recordset
ExportImgRs.Open "Select * from imagefile where imageid = " & ImgID, DB, adOpenStatic, adLockOptimistic

With frmMain.comDialog
 .Filter = "All Graphic Files|*.bmp;*.jpg;*.gif;*.emf;*.wmf|Windows Bitmaps|*.bmp|" & _
 "JPEG Filter|*.jpg|GIF Filter|*.gif|Windows Metafile|*.wmf|" & _
 "Enhanced Metafile|*.emf"
 .DialogTitle = "Export Image - Image ID = " & ImgID
 .FileName = ""
 .ShowSave
End With
TempFile = frmMain.comDialog.FileName
Pos = InStr(TempFile, ".")
 If Pos <> 0 Then
   FileStr = Left(TempFile, Pos)
   TempFile = FileStr & ExportImgRs!ImgExt
 ElseIf Pos = 0 Then
   TempFile = TempFile & "." & ExportImgRs!ImgExt
 End If
'TempFile = UCase(TempFile)
If frmMain.comDialog.FileName <> "" Then
   ReadFromDba ExportImgRs.Fields("Image"), TempFile, ExportImgRs!comp
'    m_lngPicLen = ExportImgRs.Fields("Image").ActualSize
'    If m_lngPicLen > 0 Then
'        m_strPicData = ExportImgRs.Fields("Image").GetChunk(m_lngPicLen)
'        Open TempFile For Binary As #1
'            Put #1, , m_strPicData
'        Close #1
'    Else
'    End If
End If

End Sub
